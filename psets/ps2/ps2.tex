...
Do 0 - 10 of these problems and email us your submissions by 7/23.\footnote{Mew2King should've beaten Armadad at evo.}
...
\noindent{{\bf 7: Analytic functions} Show if the following functions are analytic on C or not:
\begin{align*}
f_1(z) = \frac{z}{\overline{z}}\\
f_2(z) = e^z
\end{align*}
\medskip

\noindent{{\bf 8: Polar form and z-bar} Prove that if z has magnitude 1, $arg({\overline{z}}) = -arg(z)$ 

\medskip
\noindent{{\bf 9: Analytic function with z-bar} Prove that if f(z) is analytic on A, $\overline{f(\overline{z})}$ is also analytic on A}

\medskip
\noindent{{\bf 10*: Constant functions} Prove that if f(z) is constant in a square inside an area A and f(z) is analytic on A, f(z) is a constant function of A}
